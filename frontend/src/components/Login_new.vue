...
onMounted(async () => {
    try {
        console.log('🚀 初始化CSRF认证服务...');
        await csrfAuthService.initialize();
        csrfToken.value = csrfAuthService.csrfToken || '未获取';
        console.log('✅ CSRF认证服务初始化完成');
        
        // 检查是否已经登录
        const statusResult = await csrfAuthService.checkLoginStatus();
        if (statusResult.success && statusResult.data.is_authenticated) {
            console.log('✅ 用户已登录:', statusResult.data);
            emit('login-success', {
                user: csrfAuthService.currentUser,
...
const login = async () => {
    ...
    try {
        ...
        console.log('🔑 开始登录流程...', { 
            ...
            hasCSRFToken: !!csrfAuthService.csrfToken 
        });

        // 使用CSRF认证服务登录
        const result = await csrfAuthService.login(username.value, password.value);
    ...
    <div class="login-sidebar">
      <div class="sidebar-content">
        <div class="brand-section">
          ...
          <p class="brand-subtitle">专业的健康数据管理平台，为学生健康保驾护航</p>
        </div>
